...
import { ArrowLeft, Check, ChevronDown, Edit2, List, Trash2, RefreshCw, Star, AlertCircle, Plus, Minus, Download, Printer, MoreVertical, Video, Mic, GripVertical, AlertTriangle, BookOpen, Ticket, Wallet } from 'lucide-react';
      ...
      <div className="flex-1 p-4 lg:p-8 pt-6 lg:pt-10 max-w-7xl mx-auto w-full">
            ...
            <div className="flex-1 space-y-6 lg:space-y-8">
              ...
              <div className={`p-4 lg:p-6 rounded-2xl border-2 transition-all cursor-pointer flex flex-col sm:flex-row gap-4 lg:gap-6 items-center ${includeExtraHardcover ? 'border-accent bg-accent/5' : 'border-gray-100 bg-white hover:border-gray-200'}`} onClick={() => setIncludeExtraHardcover(!includeExtraHardcover)}>
                ...
                <div className="flex-1 w-full" onClick={(e) => e.stopPropagation()}>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="text-right">
                      <span className="text-xl font-bold text-gray-900">¥79.00/本</span>
                    </div>
                  </div>
                  <div className="relative">
                    <select
              ...
              >
                ...
                <div className="w-12 h-12 lg:w-16 lg:h-16 bg-accent/10 rounded-xl flex items-center justify-center text-accent shrink-0 transition-transform">
                  <Ticket className="w-6 h-6 lg:w-8 lg:h-8" />
                </div>
                <div className="flex-1">
              ...
              <div className="lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-md border-t border-gray-100 flex gap-3 z-50">
                <button onClick={() => setStep('preview')} className="flex-1 py-4 bg-white border border-gray-300 rounded-xl text-gray-600 font-bold">上一步</button>
                <button onClick={() => setStep('order')} className="flex-[2] py-4 bg-accent text-white rounded-xl font-bold">下一步</button>
              </div>
            </div>
            ...
            <div className="flex-1 space-y-8 lg:space-y-12">
              ...
              <div className="pt-12 flex gap-4 w-full hidden lg:flex">
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (usePoints && pointsDeduction > 0) {
                        await databaseService.spendPoints(pointsDeduction, `订购书籍抵扣: ${bookTitle}`);
                      }
                      setStep('complete');
                    } catch (error) {
                  ...
                  className="flex-2 py-4 bg-accent hover:bg-teal-700 text-white rounded-xl font-bold shadow-lg disabled:opacity-50 flex items-center justify-center space-x-2 cursor-pointer"
                >
        ...
        {step === 'complete' && (
          <div className="flex flex-col items-center justify-center h-[400px] lg:h-[600px] text-center space-y-4 lg:space-y-6 px-4">
            ...
            <p className="text-sm lg:text-base text-gray-600 max-w-md">感谢您的订购。您的长生記纪念册即将开始制作。</p>
            <button onClick={() => onComplete?.()} className="px-8 py-3 bg-primary text-white rounded-lg font-bold shadow-md">查看订单状态</button>
          </div>
        )}
      </div>
      ...
      <AnimatePresence>
        {isConfirmBackModalOpen && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-[100] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-8">
              <h2 className="text-xl font-bold text-gray-900 mb-2">确定要离开吗？</h2>
              <div className="flex space-x-3 mt-8">
                <button onClick={() => setIsConfirmBackModalOpen(false)} className="flex-1 py-3 border border-gray-200 rounded-xl font-bold text-gray-600">继续订购</button>
                <button onClick={confirmBack} className="flex-1 bg-red-500 text-white py-3 rounded-xl font-bold">确认离开</button>
              </div>
            </motion.div>
          </div>
        )}
