...
export default function RecordingFlow({ projectId, prompt, prompts = [], onSelectPrompt, onClose, onComplete, isQuickRecord = false }: RecordingFlowProps) {
  ...
  const [newStory, setNewStory] = useState<any>(null);
  ...
  const stopRecording = (): Promise<Blob | null> => {
    return new Promise((resolve) => {
      ...
      // Re-assign onstop to ensure we resolve the promise with the final blob
      recorder.onstop = () => {
        console.log("MediaRecorder stopped via stopRecording. Chunks count:", chunksRef.current.length);
        ...
        if (chunksRef.current.length === 0) {
          console.error('NoChunksError: Recording resulted in empty chunks array.');
          setRecordedBlob(null);
          resolve(null);
          return;
        }
        ...
        const blob = new Blob(chunksRef.current, { type: mimeType });
        console.log("🔵 Final Blob generated. Size:", blob.size, "Type:", blob.type);

        const url = URL.createObjectURL(blob);
        setRecordedBlob(blob);
        setRecordedUrl(url);

        // Resolve with the blob so handleNext can wait for it
        resolve(blob);
      };

      // 1. Flush any remaining data from Safari/iOS
      if (recorder.state === 'recording') {
        recorder.requestData();
      }

      // 2. Small delay to allow the last chunk to be processed by 'ondataavailable'
      setTimeout(() => {
        if (recorder.state !== 'inactive') {
          recorder.stop();
        } else {
          // If already inactive, onstop might not fire again, so we construct the blob manually if we have chunks
          if (chunksRef.current.length > 0) {
            const blob = new Blob(chunksRef.current, { type: recorder.mimeType || (mode === 'video' ? 'video/mp4' : 'audio/mp4') });
            resolve(blob);
          } else {
            resolve(null);
          }
  ...
  const handleSubmit = async () => {
    if (!recordedBlob || recordedBlob.size < 100) {
      console.error("Invalid blob detected before submit:", recordedBlob?.size);
      alert('录制似乎未成功（文件过小），请尝试重新录制。');
      setStep('ready');
      return;
    }
      ...
      (async () => {
        try {
          ...
          if (isQuickRecord && transcriptionText && !transcriptionText.includes('未检测到语音')) {
            try {
              const { optimizeStoryContent } = await import('../services/aiService');
              const aiResult = await optimizeStoryContent(transcriptionText, 'cleaned');
              if (aiResult.title) {
                updates.title = aiResult.title;
              }
              if (aiResult.content) {
                updates.content = aiResult.content;
              }
            } catch (aiErr) {
              console.error('Failed to generate AI title:', aiErr);
              updates.title = `故事回顾 - ${new Date().toLocaleDateString()}`;
            }
          } else if (isQuickRecord) {
            // Ensure title is updated even if no AI generate results, to clear placeholder
            updates.title = `故事回顾 - ${new Date().toLocaleDateString()}`;
          }
  ...
  const renderReadyStep = (isCountdown = false) => (
    <div className={`flex flex-col h-full bg-[#f5f5f0] p-4 lg:p-12 overflow-y-auto lg:overflow-hidden ${isCountdown ? 'opacity-40' : ''}`}>
      ...
      <div className="flex-1 flex flex-col max-w-2xl mx-auto w-full justify-center min-h-0">
        ...
        <div className={`bg-gray-900 rounded-3xl overflow-hidden flex-1 mb-4 relative w-full lg:max-w-md mx-auto flex items-center justify-center shadow-2xl min-h-0 ${isQuickRecord ? 'aspect-[3/4] lg:aspect-[9/16]' : 'aspect-video lg:aspect-auto'}`}>
          {!isQuickRecord ? (
            <>
            ...
            <>
              <div className="relative flex-1 h-full w-full flex items-center justify-center overflow-hidden bg-gray-900 rounded-3xl">
                {mode === 'video' ? (
                  <video
        ...
        <div className="w-full shrink-0 pb-4 pt-2 lg:pt-8 flex items-center space-x-4">
          {!isQuickRecord && (
            <div className="w-24 lg:w-32 aspect-[3/4] rounded-2xl overflow-hidden border-2 border-white shadow-lg shrink-0 bg-black flex items-center justify-center relative">
              {mode === 'video' ? (
                <video
                  ref={setVideoPreviewRef}
                  autoPlay
        ...
        return (
          <div className="flex flex-col h-full bg-[#f5f5f0] p-4 lg:p-12 overflow-y-auto lg:overflow-hidden">
            ...
            <div className="flex-1 flex flex-col max-w-2xl mx-auto w-full justify-center min-h-0">
              ...
              <div className="bg-gray-900 rounded-3xl overflow-hidden flex-1 mb-4 relative w-full lg:max-w-md mx-auto flex items-center justify-center shadow-2xl min-h-0 aspect-[3/4] lg:aspect-[9/16]">
                {/* REC Indicator */}
          ...
          <div className="flex flex-col h-full bg-[#f5f5f0] p-4 lg:p-12 overflow-y-auto lg:overflow-hidden">
            ...
            <div className="flex-1 flex flex-col max-w-2xl mx-auto w-full justify-center min-h-0 relative">
              {!isQuickRecord ? (
                ...
                <div className="bg-gray-900 rounded-3xl overflow-hidden shadow-2xl mb-4 relative flex-1 min-h-0 flex items-center justify-center aspect-[3/4] lg:aspect-[9/16] max-w-md w-full mx-auto">
              ...
              <div className={`flex items-end ${isQuickRecord ? 'justify-center' : 'justify-between'} w-full mt-4`}>
                {!isQuickRecord && (
                  <div className="w-24 lg:w-32 aspect-[3/4] rounded-2xl overflow-hidden border-2 border-white shadow-2xl bg-black relative shrink-0">
                    {mode === 'video' ? (
                      <video
                        ref={setRecordingPreviewRef}
                        autoPlay
      ...
      case 'review':
        return (
          <div className="flex flex-col h-full bg-[#f5f5f0] p-6 lg:p-12 overflow-y-auto lg:overflow-hidden">
            ...
            <div className="flex-1 flex flex-col max-w-md mx-auto w-full min-h-0 items-center">
              <div className="bg-gray-900 rounded-3xl overflow-hidden mb-4 relative shadow-xl flex-1 w-full min-h-0 flex items-center justify-center aspect-[3/4] lg:aspect-[9/16]">
                {mode === 'video' ? (
                  recordedUrl && (
                    <video
                      key={recordedUrl}
              ...
              <div className="grid grid-cols-2 gap-4 shrink-0 pb-4 w-full">
                <button
          ...
          <div className="flex flex-col h-full bg-[#0d1b1e] text-white p-8 lg:p-12 justify-center">
            <div className="max-w-xl mx-auto w-full space-y-12 text-center">
              ...
              <div className="space-y-4 pt-8">
                ...
                <button
                  onClick={() => {
                    onComplete?.({ storyId: newStoryId, story: newStory });
                  }}
