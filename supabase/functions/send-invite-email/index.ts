...
serve(async (req) => {
  ...
  try {
    const { email, projectId, inviterName, projectTitle } = await req.json();

    if (!email || !projectId) {
      throw new Error('Missing email or projectId');
    }
    ...
    const finalLink = `https://everstory.ai?inviteProjectId=${projectId}`;
    ...
    if (!res.ok) {
      ...
      throw new Error(`Resend API Error: ${res.status} ${res.statusText} - ${errorData}`);
    }
